...
import { getEstoqueGlobalLogsWithDrinkNames } from "@/services/estoqueGlobalService";
...
export default function HistoricoEstoqueGlobalPage() {
  const t = useTranslations("adminEstoqueGlobalHistorico");
  ...
  async function fetchLogs() {
    try {
      const data = await getEstoqueGlobalLogsWithDrinkNames();
      logs$.set(data);
    } catch {
  ...
  const finalBalanceByDrink = useMemo(() => {
    ...
    filteredLogs.forEach((log) => {
      ...
      balance[log.drink] = current + Number(log.quantity);
    });
    return balance;
  }, [filteredLogs]);
  ...
  const gerarPDF = () => {
    ...
    if (startDate || endDate || drinkFilter) {
      ...
      const periodStart = startDate
        ...
        : t("export.periodStart");
      const periodEnd = endDate
        ? format(endDate, "dd/MM/yyyy", { locale: ptBR })
        : t("export.periodEnd");
      const drinkFilterText = drinkFilter
        ? t("export.drinkFilter", { drink: drinkFilter })
        : "";
      doc.text(
        t("export.period", {
          start: periodStart,
          end: periodEnd,
          drink: drinkFilterText,
        }),
        14,
        27
      );
    ...
    <div className="p-6 max-w-6xl mx-auto">
      ...
      <ul className="list-disc ml-6">
        {Object.entries(finalBalanceByDrink).map(([drink, balance]) => (
          <li key={drink}>
            <strong>{drink}:</strong> {balance}{" "}
            {balance !== 1 ? t("balance.units") : t("balance.unit")}
          </li>
        ))}
      </ul>
    </div>
